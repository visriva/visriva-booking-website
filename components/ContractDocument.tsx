...
interface Props {
  ...
  loading?: boolean;
  /** Persist signature (Firestore). If omitted, local-only sign. */
  onSign?: (signatureName: string) => Promise<{ success: boolean; error?: string }>;
}

export default function ContractDocument({ data, loading, onSign }: Props) {
  const [signatureName, setSignatureName] = useState(data.signatureName || "");
  const [termsAccepted, setTermsAccepted] = useState(false);
  const [isSigned, setIsSigned] = useState(
    data.status === "signed" || Boolean(data.signatureName && data.signedAt)
  );
  const [signedName, setSignedName] = useState(data.signatureName || "");
  ...
  const invoiceNumber = data.invoiceNumber || "VIS-DRAFT";
  ...
  const handleSignAgreement = async (e: React.FormEvent) => {
    ...
    if (onSign) {
      setSigning(true);
      const res = await onSign(signatureName.trim());
      setSigning(false);
      if (!res.success) {
        setSignError(res.error || "Failed to save signature");
        return;
      }
    }
  ...
  const handlePayDeposit = () => {
    const message = `Hello Visriva Team! I am confirming the booking for *${data.clientName}* (${data.eventType} on ${data.eventDate} at ${data.venue}). I have reviewed Invoice *${invoiceNumber}* for Total Amount ₹${totalAmount.toLocaleString("en-IN")}. Please share the UPI / Bank details for 40% Deposit (₹${depositAmount.toLocaleString("en-IN")}).`;
    window.open(`https://wa.me/918884484828?text=${encodeURIComponent(message)}`, "_blank");
  };
        ...
        <div className="bg-[#041a12] border-2 border-[#D4AF37]/50 rounded-3xl p-6 sm:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.8)] text-white space-y-10 print:bg-white print:text-black print:border-none print:shadow-none print:p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/30 p-6 rounded-2xl border border-white/10 print:bg-gray-50 print:border-gray-300">
            <div className="space-y-2">
              ...
              {data.companyName && (
                <div className="text-xs text-white/80 font-mono">
                  Attn: <strong>{data.clientName}</strong>
                </div>
              )}
              {data.isGstInvoice && data.companyGstin && (
                <div className="text-xs text-[#D4AF37] font-mono font-bold">
                  GSTIN: <span className="uppercase">{data.companyGstin}</span>
                </div>
              )}
              {data.customHashtag && (
                <div className="text-xs text-emerald-300 font-mono">
                  Hashtag/Monogram: <strong>{data.customHashtag}</strong>
                </div>
              )}
              <div className="space-y-1 text-xs text-emerald-100/80 font-mono print:text-gray-700 pt-1">
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    Event Date: <strong>{data.eventDate}</strong>
                  </span>
                </div>
                <div className="flex items-center space-x-2">
            ...
            <div className="space-y-2 flex flex-col justify-between">
              <div>
                ...
                <h3 className="font-serif text-lg font-bold text-white print:text-black">{data.eventType}</h3>
                <div className="space-y-1 text-xs text-emerald-100/80 font-mono print:text-gray-700">
                  <div>
                    Services Included:{" "}
                    <strong>{data.services.join(", ") || "Instant Photo Booth"}</strong>
                  </div>
                  <div>
                    Operational Hours: <strong>Up to 4 Hours Live Printing</strong>
                  </div>
                </div>
              </div>
              ...
              {data.clientLogoUrl && (
                <div className="pt-2">
                  ...
                  <div className="w-16 h-12 rounded-lg border border-[#D4AF37]/40 bg-black/50 p-1 mt-1">
                    ...
                    <img src={data.clientLogoUrl} alt="Brand Logo" className="w-full h-full object-contain" />
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            <h3 className="font-serif text-lg font-bold text-white border-b border-white/10 pb-2 print:text-black print:border-gray-300">
              {data.isGstInvoice
                ? "Official Tax Invoice & Payment Breakdown (18% GST)"
                : "Itemized Quotation & Payment Schedule"}
            </h3>
              ...
              <table className="w-full text-left border-collapse text-xs font-mono">
                ...
                <tbody className="divide-y divide-white/10 print:divide-gray-200">
                  {data.isGstInvoice ? (
                    <>
                      <tr>
                        <td className="py-3 px-2">
                          <strong className="text-white block print:text-black">
                            {data.eventType} Live Setup (Taxable Subtotal)
                          </strong>
                          <span className="text-[11px] text-emerald-100/60 block print:text-gray-600">
                            {data.services.join(" + ")} with 2 uniformed operators
                          </span>
                        </td>
                      ...
                      <tr>
                        ...
                        <td className="py-3 px-2 text-right text-white/80">
                          ₹{Math.round((totalAmount / 1.18) * 0.09).toLocaleString("en-IN")}
                        </td>
                      </tr>
                      <tr>
                    ...
                    <tr>
                      <td className="py-3 px-2">
                        <strong className="text-white block print:text-black">{data.eventType} Live Setup</strong>
                        <span className="text-[11px] text-emerald-100/60 block print:text-gray-600">
                          {data.services.join(" + ")} with 2 uniformed operators &amp; 8-second dye-sub printer
                        </span>
                      </td>
                      <td className="py-3 px-2">1 Event</td>
                      <td className="py-3 px-2 text-right font-bold text-white print:text-black">
          ...
          <div className="space-y-3 pt-4 border-t border-white/10 text-xs text-emerald-100/80 leading-relaxed font-sans print:text-gray-700">
            <h4 className="font-serif font-bold text-sm text-white print:text-black">
              Key Service Rules &amp; Responsibilities
            </h4>
            <ul className="space-y-2 list-disc list-inside">
              <li>
                <strong>Travel &amp; Distance Policy:</strong> Free transport &amp; logistics within{" "}
                <strong>15 km</strong> from Bengaluru City Center. Venues beyond 15 km are billed travel &amp;
                toll at actuals.
              </li>
              <li>
                <strong>Power Access:</strong> Venue organizer must provide 1 dedicated 5A / 230V standard power
                plug point within 15 meters of setup location.
              </li>
              <li>
                <strong>Setup Time:</strong> Visriva technical staff requires access to the venue 60–90 minutes
                prior to event reporting time.
              </li>
              <li>
                <strong>Booking Confirmation:</strong> Dates are reserved upon receipt of the 40% advance
                deposit.
              </li>
            </ul>
          </div>
          ...
          <div className="pt-6 border-t border-[#D4AF37]/30 space-y-4">
            ...
            {isSigned ? (
              <div className="p-4 rounded-2xl bg-emerald-500/20 border border-emerald-400/50 text-emerald-300 font-mono text-xs space-y-1">
                ...
                <div>
                  Signed By: <strong>{signedName || data.signatureName}</strong>
                </div>
                <div>Signed Date: {signedDateStr}</div>
              </div>
            ) : (
              <form onSubmit={handleSignAgreement} className="space-y-4 print:hidden">
                ...
                {signError && <p className="text-xs text-rose-300">{signError}</p>}

                <button
                  type="submit"
                  disabled={!signatureName.trim() || !termsAccepted || signing}
                ...
                >
                  {signing ? "Saving…" : "Sign & Accept Agreement"}
                </button>
              </form>
            )}
          </div>
